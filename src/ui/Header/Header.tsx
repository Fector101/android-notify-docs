...
export default function Header() {
    ...
    const isMobileView = useRef(typeof window !== 'undefined' && window.innerWidth < 770)
        ...
        <header className='flex align-items-cen width100per'>
            ...
            <div className='css-dropdown flex fd-column'>
                <span className='flex active'>
                    <Link href='/versions' className='version-no'>v{version}</Link>
            ...
            <p className='page-title'>{pathname.slice(1).split('-').map(toTitleCase).join(' ')}{description || ''}</p>
            {/* <input value={search} onChange={(e)=>setSearch(e.target.value)}/> */}
            <nav className="flex margin-left-auto icon-nav">
                <a target='_blank' rel='noopener noreferrer' className='btn-link' href="https://github.com/Fector101/android_notify"> <Github /> </a>
